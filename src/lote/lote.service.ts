...
export default class LoteService{
    ...
    async associatePropietario(lote_id:string, barrio_id:string, propietario_id:string):Promise<boolean>{
    ...
    async getAll(barrio_id:string){
        ...
        const propietariosOfLotes:any[] = await this.loteRepo.query(select_propietarios_of_lotes(lote_ids))
...
function insert_propiertario_de_lote_query(lote_id:string, barrio_id:string, propietario_id:string):string {
